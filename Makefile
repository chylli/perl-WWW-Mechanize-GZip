# This Makefile is for the WWW::Mechanize::GZip extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/WWW/Mechanize/GZip.pm]
#     AUTHOR => [q[Peter Giessner <cardb@planet-elektronik.de>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[WWW::Mechanize::GZip]
#     PL_FILES => {  }
#     PREREQ_PM => { Compress::Zlib=>q[0], Test::More=>q[0], WWW::Mechanize=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/WWW/Mechanize/GZip.pm]
#     clean => { FILES=>q[WWW-Mechanize-GZip-*] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/WWW/Mechanize/GZip.pm
...
TO_INST_PM = lib/WWW/Mechanize/GZip.pm

PM_TO_BLIB = lib/WWW/Mechanize/GZip.pm \
...
DISTNAME = WWW-Mechanize-GZip
DISTVNAME = WWW-Mechanize-GZip-0.12
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/WWW/Mechanize/GZip.pm $(INST_MAN3DIR)/WWW::Mechanize::GZip.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''tries to fetch webpages with gzip-compression'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Peter Giessner <cardb@planet-elektronik.de>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: WWW-Mechanize-GZip' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "tries to fetch webpages with gzip-compression",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Peter Giessner <cardb@planet-elektronik.de>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "WWW-Mechanize-GZip",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>tries to fetch webpages with gzip-compression</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Peter Giessner &lt;cardb@planet-elektronik.de&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Compress::Zlib" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/WWW/Mechanize/GZip.pm blib/lib/WWW/Mechanize/GZip.pm 
